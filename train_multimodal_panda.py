...
class Panda70MVideoDataset(Dataset):
    """Load video clips from Panda-70M dataset.

    Each video has a companion JSON with timestamps and captions.
    Clips are sampled from annotated segments rather than the full video.
    """
    ...
    def __init__(self, root: str, resolution: int = 128, n_frames: int = 8):
        ...
        # Find all video files (recursively, handles panda70m/data/ structure)
        ...
        # Build clip index: (video_path, start_time_sec, end_time_sec, caption)
        self.clips = self._build_clip_index()
        log.info("Panda70MVideoDataset: %d clips from %d videos at %dpx, %d frames",
                 len(self.clips), len(self.video_paths), resolution, n_frames)

    def _parse_timestamp(self, ts: str) -> float:
        """Parse 'H:MM:SS.mmm' timestamp to seconds."""
        parts = ts.split(":")
        if len(parts) == 3:
    ...
    def _build_clip_index(self):
        """Build a flat list of (video_path, start_sec, end_sec, caption) from JSON metadata."""
        clips = []
        for vpath in self.video_paths:
            json_path = vpath.with_suffix(".json")
            if json_path.exists():
                try:
                    with open(json_path, "r") as f:
                        meta = json.load(f)
                    timestamps = meta.get("timestamp", [])
                    captions = meta.get("caption", [])
                    for i, (start_end) in enumerate(timestamps):
                        start_sec = self._parse_timestamp(start_end[0])
                        end_sec = self._parse_timestamp(start_end[1])
                        cap = captions[i] if i < len(captions) else ""
                        if end_sec - start_sec >= 0.5:  # skip very short clips
                            clips.append((vpath, start_sec, end_sec, cap))
                except Exception:
                    # Fallback: use entire video as one clip
                    clips.append((vpath, 0.0, -1.0, ""))
            else:
                clips.append((vpath, 0.0, -1.0, ""))

        # If no clips found from metadata, fall back to whole-video entries
        if not clips:
            clips = [(p, 0.0, -1.0, "") for p in self.video_paths]
        return clips
    ...
    def __getitem__(self, idx):
        vpath, start_sec, end_sec, caption = self.clips[idx]
        try:
            frames = self._load_clip(vpath, start_sec, end_sec)
        except Exception:
            frames = torch.zeros(3, self.n_frames, self.resolution, self.resolution)
        return frames  # (3, T, H, W)

    def _load_clip(self, path: Path, start_sec: float, end_sec: float) -> torch.Tensor:
        import cv2
        ...
        clip_len = end_frame - start_frame + 1

        # Uniform sampling within the clip segment
        T = self.n_frames
        if clip_len < T:
            indices = [start_frame + i for i in range(clip_len)]
            indices += [indices[-1]] * (T - clip_len)
        else:
            indices = [int(i) for i in torch.linspace(start_frame, end_frame, T)]
    ...
    def setup(self, stage=None):
        if self.synthetic:
            ...
            self.val_video = SyntheticMultiModalDataset(
                max(8, self.synthetic_n // 10), "video", self.video_res,
                self.video_frames)
            ...
            try:
                self.train_video = Panda70MVideoDataset(
