"""Stage 5a: AsymmetricDecoder — z → pixel reconstruction.

The understanding head moved into ``matryoshka_head.MatryoshkaHead`` (it now
attention-pools the C-D-split tokens directly, not the VAE bottleneck), so
this module hosts only the reconstruction path:
