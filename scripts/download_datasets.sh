...
PY
    ...
    # Use Python snapshot_download with allow_patterns list — `hf download`
    # CLI silently honors only the LAST --include flag when many are passed.
    log "Tải ${keep} shard về ${local_dir} (snapshot_download)..."
    HF_REPO="${repo_id}" \
    HF_LOCAL_DIR="${local_dir}" \
    HF_KEEP_FILE="${local_dir}/.shards_keep.txt" \
    HF_MAX_WORKERS="${NUM_PROC}" \
    python3 - <<'PY'
import os
...
keep_file = os.environ["HF_KEEP_FILE"]
max_workers = int(os.environ.get("HF_MAX_WORKERS") or 8)
token = os.environ.get("HF_TOKEN") or None

with open(keep_file, encoding="utf-8") as f:
    patterns = [ln.strip() for ln in f if ln.strip()]
print(f"[snapshot_download] {repo} -> {local_dir} ({len(patterns)} patterns, workers={max_workers})")
snapshot_download(
    repo_id=repo,
    repo_type="dataset",
    local_dir=local_dir,
...
fetch_webvid_videos() {
    ...
    local out_dir="${WEBVID_VIDEO_OUT:-${WEBVID_DIR}/videos_train}"
